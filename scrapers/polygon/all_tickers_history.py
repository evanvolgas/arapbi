...
LOG_NAME = "polygon_all_tickers_history"
    ...
    if aggs:
        daily  = [{ 'ticker': y.ticker,
                    ...
                    'transactions': y.transactions,
                     'date': d } for y in aggs]
        df = pd.DataFrame(daily)
        df.to_csv(f"gs://arapbi-polygon/polygon/tickers/{d}.csv")
        # logger.log_text(f'Assembled dataframe of stock history data for {d}')
        print(f'Assembled dataframe of stock history data for {d} and uploaded it to Google Storage')
        return df
    else:
        # logger.log_text(f'No stocks data for {d}')
        print(f'No stocks data for {d}')
...
if __name__ == "__main__":
    ...
    #dates = [str(d)[:10] for d in pd.date_range('2018-10-12', today)]
    ...
    files = list(storage_client.list_blobs(bucket_or_name=BUCKET_NAME, prefix=GCP_FOLDER_NAME))
    ...
    # logger.log_text(f'Making stocks DataFrame')
    ...
    all_tickers_history['volume_weighted_average_price'] = all_tickers_history['volume_weighted_average_price'].fillna(0).astype(float)
    all_tickers_history['volume'] = all_tickers_history['volume'].fillna(0).astype(int)
    all_tickers_history['transactions'] = all_tickers_history['transactions'].fillna(0).astype(int)
    ...
    pandas_gbq.to_gbq(all_tickers_history,
                    BQ_TABLE_NAME,
