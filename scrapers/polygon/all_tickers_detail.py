...
LOG_NAME = "polygon_all_tickers_detail"
...
def fetch_ticker_details(ticker):
    ...
    print(f'Retrieving Ticker Details for {ticker}')
    ...
    if aggs:
        t  = [{ 'active': y.active,
                    ...
                    'type': y.type,
                    'weighted_shares_outstanding': y.weighted_shares_outstanding} for y in aggs]
        df = pd.DataFrame(t)

        # logger.log_text(f'Assembled dataframe of stock history for {ticker} // {title}', )
        print(f'Assembled dataframe of ticker details for {ticker}')
        with dfs_lock:
            dfs.append(df)
    else:
        # #logger.log_text(f'No data for {ticker} on behalf of {title}')
        # logger.log_text(f'No stocks data for {ticker} on behalf of {title}')
        print(f'No Response')
...
if __name__ == "__main__":
    ...
    all_stocks['share_class_shares_outstanding'] = all_stocks['share_class_shares_outstanding'].fillna(0).astype(int)
    ...
    all_stocks['weighted_shares_outstanding'] = all_stocks['weighted_shares_outstanding'].fillna(0).astype(int)
